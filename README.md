...
本系统为原型验证，若应用于生产环境，以下维度需要进一步优化。

**识别准确度优化：**

Prompt 工程方面，当前 System Prompt 列举了 nginx、apache 等示例服务名，会导致模型对未列举的服务（Tomcat、IIS、Jetty、Elasticsearch）识别偏向不足，应去掉示例列表让模型自由判断，同时可加入 2-3 个 few-shot 示例覆盖标准格式和伪装格式，让模型学到输出规范和识别策略，代价是增加 token 消耗。

Banner 预处理方面，当前原始 Banner 直接传入 LLM 未做清洗，实际场景中可能包含控制字符、冗余头信息等噪音。预处理去掉无关字符、统一大小写、提取关键行，既减少 token 消耗又提升识别准确率。

Embedding 模型方面，当前使用的 all-MiniLM-L6-v2 是 384 维英文模型，对中文安全知识（漏洞描述、SOP 内容）语义理解较弱。可替换为 bge-small-zh 或 multilingual-e5-small，提升中英文混合知识库的检索效果。

置信度校准方面，LLM 自评的 confidence 值可靠性有限，常对错误识别也给出高置信度。可通过批量测试结果统计各 confidence 区间的实际准确率，建立校准曲线将原始值映射到校准值。

多模型投票方面，高价值资产场景可用两个不同模型分别识别取共识结果，一致时输出，不一致时降低 confidence 并标记"需人工确认"，代价是延迟和成本翻倍。

知识库质量方面，当前 11 条预置知识覆盖面有限，生产环境需扩充至数百条以上。知识内容质量直接决定 RAG 检索效果，建议建立入库审核机制，确保关键词覆盖和描述准确性。

测试数据集方面，20 条测试数据统计意义不足，应扩充至 100 条以上，覆盖更多服务类型、伪装场景和边界 case（空输入、超长 Banner、非英文等）。版本号匹配应做规范化处理，提取主版本.次版本忽略 patch 后缀，避免 "8.9p1" 与 "8.9" 判为不同。

**项目工程优化：**

鉴权与审计方面，API 当前无认证机制，需加 API Key 或 JWT 鉴权，不同角色区分只读与编辑权限。操作行为（谁在何时识别了什么、谁修改了哪条知识）需记录独立审计日志且不可篡改。

数据存储方面，JSON 文件存储在条目增多后存在并发安全和性能问题，最低成本迁移方案为 SQLite（零部署、单文件、支持事务），多实例部署场景再升级 PostgreSQL。

LLM 调用方面，OpenAI client 应改为单例复用连接，增加指数退避重试（429 限流、网络抖动时重试 2-3 次）和超时控制（30 秒主动断开）。批量测试应并发执行（asyncio.gather + 信号量限并发 5），将 20 条 40 秒压至 10 秒以内。相同 Banner 识别结果可加缓存层（hash → 结果，TTL 1 小时，知识库更新时清空），避免重复调用浪费 token。

监控告警方面，需接入 Prometheus 采集 QPS、平均响应时间、LLM 调用成功率、RAG 命中率等指标，配 Grafana 仪表盘，LLM 错误率超限或 RAG 命中率突降时自动告警。

安全防护方面，用户输入 Banner 应做长度限制和清洗防止 Prompt 注入，知识库 content 字段同样需要长度约束。API 应加 rate limiter（单 IP 每分钟 10 次识别、批量测试每小时 5 次），防止恶意调用烧光 API 配额。

测试覆盖方面，`_parse_and_fix` 等核心解析逻辑需补充单元测试，覆盖纯文本、Markdown 包裹、JSON 缺字段、非法值等异常输出。RAG 检索需集成测试，验证特定 query 的召回结果和相似度排序。
